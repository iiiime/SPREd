...
def gen_input(expr, idx, n_features, weights):
	...
	cov = np.cov(x, ddof=1, aweights=weights)
	cov_s = args.lambda_ * np.eye(x.shape[0]) + (1. - args.lambda_) * cov # shrinkage
	...
	pm = np.linalg.inv(np.array(cov_s) + 0.001 * np.identity(l)) # precision matrix
	m = np.abs(pm) # gene bias correction
	tf_idx = np.arange(args.n_features)
	...
	out.append(m[upper_triangular_idx]) # corrected precision matrix
...
def main():
	...
	for n in range(args.file_id * args.n_samples, args.file_id * args.n_samples + args.n_samples):
		...
		if args.n_cond > expr.shape[1]: # n_cond > n_genes
			_P = 1. - scipy.spatial.distance.cdist(expr, expr, metric='correlation')
			theta = 0.8
			counts = np.zeros(args.n_cond)
			idx = np.where(_P > theta)
			np.add.at(counts, idx[0], 1)
			np.add.at(counts, idx[1], 1)
			weights = 1.0 / (1. + np.asarray(counts, dtype=float))
		else:
			weights = np.ones(args.n_cond) / args.n_cond
		...
		with open(inter, 'r') as f:
			for line in f:
				...
				if idx < n_features + args.n_mrs:
					continue
				out = gen_input(expr, idx, n_features, weights)
				hist.append(out)
				#print(idx)
	...
	label = label.reshape(-1, n_features) #TODO
